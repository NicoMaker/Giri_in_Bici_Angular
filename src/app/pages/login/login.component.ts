...
export class LoginComponent {
  ...
  inCorso = false;
  messaggio: { testo: string; tipo: 'success' | 'error' | 'warning' } | null = null;
  cookieAccettati = false;
  scuoti = false;

  async accedi(): Promise<void> {
    if (!this.cookieAccettati) {
      this.messaggio = { testo: '⚠️ Devi accettare i cookie per accedere!', tipo: 'warning' };
      return;
    }
    this.inCorso = true;
    this.messaggio = null;
    ...
    try {
      ...
      if (ok) {
        this.messaggio = { testo: '✅ Accesso riuscito! Reindirizzamento...', tipo: 'success' };
        await this.router.navigate(['/giri']);
      } else {
        this.messaggio = { testo: '❌ Nome utente o password non validi!', tipo: 'error' };
        this.scuoti = true;
        setTimeout(() => (this.scuoti = false), 500);
      }
    } catch (e) {
      console.error("Errore durante l'accesso:", e);
      this.messaggio = { testo: "❌ Errore durante l'accesso. Riprova più tardi.", tipo: 'error' };
    } finally {
      this.inCorso = false;
    }
  }

  ...
  async ngOnInit(): Promise<void> {
    ...
    const risolte = await Promise.all(
      seasonsData.seasons.map(async (season) => {
        ...
        const flat = datiPeriodi
          .filter((d): d is PeriodEntry[] => Array.isArray(d))
          .flat();
        return {
          name: season.name,
    ...
    const somma = (arr: PeriodEntry[]) =>
      arr.reduce((s, r) => s + (r.distance || 0), 0);
    ...
    this.avgPeriod = formatNumber(
      totalePeriodi > 0 ? totale / totalePeriodi : 0,
    );
    this.caricamento = false;
  }
}

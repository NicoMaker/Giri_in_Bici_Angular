...
export class StatisticheMensiliComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    const totaleCorse = allData.reduce(
      (total, json) => total + (json?.numberOfRaces || 0),
      0,
    );
    ...
    this.righe = elencoMesi.map((mese, i) => ({
      ...
      percentuale: formatNumber(
        totaleChilometri > 0
          ? (chilometriTotali[i] / totaleChilometri) * 100
          : 0,
      ),
      mesiPercorsi: mesiPercorsi[i],
      kmMedioMese:
        mesiPercorsi[i] > 0
          ? formatItalianNumber(chilometriTotali[i] / mesiPercorsi[i], true)
          : "0,00",
